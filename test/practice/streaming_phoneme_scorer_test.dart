...
class _FakeTemplateSource implements VocalTemplateSource {
  ...
  final Map<VocalSlot, VocalTemplate> templates;

  @override
  Future<VocalTemplate> templateFor(VocalSlot word) async => templates[word]!;
...
final Map<VocalSlot, ({double start, double end})> _bands = {
  ...
  // The two openers sweep DOWNWARD. Six same-shaped upward chirps stopped
  // being separable once a sixth word joined: the scorer drops C0 and applies
  // CMN, which normalises away much of a narrow chirp's absolute band, so the
  // words were distinguished mostly by sweep shape — and five was all that
  // fit. Reversing the openers' direction restores a wide margin, and it
  // mirrors the real vocabulary, where the openers are the least
  // element-like words.
...
Uint8List _wordAudio(VocalSlot word, {int samples = 6400}) {
  final band = _bands[word]!;
  return _chirpPcm(samples, startFreq: band.start, endFreq: band.end);
}

void main() {
  final templates = <VocalSlot, VocalTemplate>{
  ...
  test('feeding audio identical to the reference completes the formula', () async {
    ...
    await scorer.beginFormula(const PracticeFormula([VocalSlot.fire]));
    ...
    for (int rep = 0; rep < 5 && !scorer.isComplete; rep++) {
      feed(scorer, _wordAudio(VocalSlot.fire));
    }
  ...
  test('the WRONG vocabulary word stalls the pointer (contrastive margin)',
    ...
    await scorer.beginFormula(const PracticeFormula([VocalSlot.water]));

    for (int rep = 0; rep < 5 && !scorer.isComplete; rep++) {
      feed(scorer, _wordAudio(VocalSlot.earth));
    }

    expect(scorer.isComplete, isFalse);
    expect(scorer.currentTarget?.label, 'water');

    // ...and the stall hint should finger the word actually spoken.
    expect(scorer.currentBestGuess?.label, 'earth');
    ...
    for (int rep = 0; rep < 5 && !scorer.isComplete; rep++) {
      feed(scorer, _wordAudio(VocalSlot.water));
    }
    expect(scorer.isComplete, isTrue);
    scorer.dispose();
  });
  ...
  test('pure digital silence never crosses (energy gate)', () async {
    final scorer = buildScorer();
    await scorer.beginFormula(const PracticeFormula([VocalSlot.air]));
  ...
  test('minimum-audio guard: no evaluation until enough fresh audio arrives',
    ...
    final refFrames = templates[VocalSlot.fire]!.mfccFrames.length;
    ...
    feed(scorer, _wordAudio(VocalSlot.fire, samples: fewSamples));
  ...
  test('debounce requires sustained clearance, not a single lucky frame', () async {
    ...
    scorer.acceptPcmChunk(_wordAudio(VocalSlot.fire, samples: 160));
