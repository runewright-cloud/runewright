...
void main() {
  group('InkStep.step', () {
    test('rule B alone: a 2-cell straight stroke grows by one cell at each end', () {
      const rules = InkRules(ruleA: false, ruleB: true, ruleC: false, ruleE: false);
    ...
    test('rule C: two crossing strokes burst at the intersection, and the burst ring does not immediately re-burst', () {
      const rules = InkRules(ruleA: false, ruleB: false, ruleC: true, ruleE: false);
      // Axis A run and axis C run crossing at the origin -- both axes are
      // already complete at the origin in the seed itself.
      final seed = <HexCoord>{
        const HexCoord(-1, 0), const HexCoord(0, 0), const HexCoord(1, 0),
        const HexCoord(-1, 1), const HexCoord(1, -1),
      };

      final gen1 = InkStep.step(active: seed, radius: 8, generation: 1, rules: rules);

      // Burst activates the origin and all 6 of its neighbors.
      for (final dir in InkStep.directions) {
        expect(gen1, contains(HexCoord(dir.q, dir.r)));
      }

      final gen2 = InkStep.step(active: gen1, radius: 8, generation: 2, rules: rules);
      // None of the newly-activated ring cells have 2 complete axes of
      // their own, so no further cells should be activated this step.
      expect(gen2, equals(gen1));
    });

    test('rule C: a bent stroke does not burst at the corner; both free ends extend straight under rule B', () {
      const rules = InkRules(ruleA: false, ruleB: true, ruleC: true, ruleE: false);
      ...
      // directions, so it has 0 complete axes -- no burst.
    ...
    test('rule A: a 1-cell gap between two collinear approaching strokes is filled', () {
      const rules = InkRules(ruleA: true, ruleB: false, ruleC: false, ruleE: false);
      final seed = <HexCoord>{
        const HexCoord(-3, 0),
        const HexCoord(-2, 0),
        const HexCoord(0, 0),
