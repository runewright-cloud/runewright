...
| `bookLeafCount` | 0x1C | both→both | uint32 big-endian | LAN duel setup only — each side's chapter leaf count, so `DrawSchedule` can compute `nextInt(n)` for the peer's chapter without learning its contents |
| `wizardName` | 0x1D | both→both | UTF-8 bytes, may be empty | unauthenticated, presentation only — never trusted for identity or authorization |
| `avatarId` | 0x1E | both→both | UTF-8 bytes, may be empty | unauthenticated, presentation only (docs/AVATAR_PICKER_PLAN.md §5.2); an unknown id degrades to the default |
| `armorLoadout` | 0x1F | both→both | JSON: `{"armor": null}` or `{"armor":{"tier":int,"proofB64":str}}` | **protocol v6.** LAN duel setup only — the player's equipped Aetherial Armor (docs/AETHERIAL_ARMOR.md). Sent on EVERY duel: `{"armor":null}` is a declaration, not a silence, because a conditional frame leaves the two peers in different handshake states. Carries the proof plus a ROUTING tier and nothing else — no `isArmor`, formula, mana cost, supreme tags, counts, bonuses, keywords or slot cost, all of which are re-derived from the verified public outputs on both devices (`armor_certification.dart`). The declared tier selects the VK/public-input layout before the proof's T is readable; after parsing, the certified T must imply that same tier or the match aborts |
...
| `setupReady` | 0x47 | both→both | empty | **protocol v7.** LAN duel setup only — the setup-finalization barrier (docs/AETHERIAL_ARMOR.md §3d). Sent by each side once EVERY setup validation has passed (armor certified, slot budget recomputed, permissions verified) and awaited before either side builds a `BattleState`. Asserts nothing but "I found no reason to refuse this match". Mandatory, like `armorLoadout` — a conditional barrier is not a barrier. Setup validation is asymmetric in time, so without it a rejected match could leave one device in a battle screen and the other in the lobby; every refusal path forfeits, and a forfeit interrupts a blocked typed wait, so a refusal on either side now aborts both. Allocated in the 0x4x block because 0x10–0x1F is full |
